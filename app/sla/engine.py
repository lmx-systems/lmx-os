...
# Hub 1 calibration - see module docstring.
...
# heuristics, when a POS adapter is able to surface them.
...
def classify_tier(order: NormalizedOrder) -> tuple[str, str]:
    ...
    Returns (tier, reason). Heuristic order:
      1. Explicit rush flag from the POS payload -> T1.
      2. Explicit scheduled/will-call flag -> T3.
      3. Otherwise -> T2 (the default, standard-urgency bucket).
    """
