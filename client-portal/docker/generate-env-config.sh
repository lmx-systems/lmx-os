...
# pointing this portal at a different API is a container restart with a
# different `environment:` value, not a Docker image rebuild
# (docs/ROADMAP.md D2) - see index.html/src/lib/api.ts for how it's
# consumed.
set -eu

cat <<EOF > /usr/share/nginx/html/env-config.js
window.__RUNTIME_CONFIG__ = {
  VITE_API_BASE_URL: "${CLIENT_PORTAL_API_BASE_URL:-http://localhost:8000}"
};
EOF
