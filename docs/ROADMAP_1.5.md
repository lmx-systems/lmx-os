...
| **DRV-7** | Dock survey at the stop — the writer `IDN-4`'s surveyed columns never had | `BUILT` | `app/identity/dock_survey.py`, `POST /driver/stops/{id}/dock-survey`, `DockSurveyModal`, migration `0065`. **Seven allowlist entries came off in the same change**: `set_access` and `set_autonomy_fit` from the orphan list, and `curb_access`, `door_path`, `landing_surface`, `obstruction`, `who_receives` from the write-only-column list — all of them complete, validated and tested, and written by nothing since the day they were added. **The decision is server-side.** `dock_needs_survey` weighs the 365-day interval, the three-per-shift cap and the stop type together, and the app renders what it is told; restating any of it on the phone is the mistake `THE_DRIVER_APP.md` §6 records twice. **It opens after `complete_stop` is queued, never before** — a measurement may fail, a delivery may not — every question has a skip, an empty submission is a valid 200, and the whole thing goes through the outbox as a sixth action type. **`stop_point` is the one question that had no column** (`STOP_POINTS`, migration `0065`), and `none_legal` is a real answer: a dock a van cannot lawfully stop at is exactly what an autonomy programme needs to know, and rounding it to `street_legal` would hide it. `surveyed_by_driver_id` is a foreign key so a bad surveyor can be found and their rows discounted — these become `M5`'s labels. A value outside a vocabulary is a 422 naming the field, not a stored surprise; a stop whose address names no place is a 409 rather than a silent write to `IDN-1`'s deliberately-null dock. **Not yet done:** the 80% coverage figure the done-when names is measured after go-live. **The standalone Dock Log is built** (25 Sep) — `client-portal/src/components/DockLogPage.tsx` at `/dock-log`, `POST /public/dock-log`, `app/identity/dock_log.py`, migration `0066`. Both conditions this row set are met. The answer set **is** these vocabularies rather than a translation of them — `ANSWER_VOCABULARIES` validates against the same tuples `DockSurveyModal` uses, and a mismatch is a 422 naming the field. And a stranger's submission **cannot** reach `receiver_profiles`: it lands in `dock_log_submissions`, which nothing operational reads, and moves only through `DockLogReviewPanel` — an operator picks the dock, because coordinates are as freely chosen as a business name, so the ~150 m shortlist is a suggestion and never a match. `import_submission` **refuses** rather than skips in all three cases (unmatched, already imported, already surveyed by one of ours); a skip returning success would be indistinguishable from an import that worked. Answers land as `SOURCE_STATED` in a new `access_source` column and deliberately do **not** stamp `surveyed_at`, so `is_surveyed` stays false and the next driver who delivers there is still asked — copying the answers is easy, copying them in a way that convinces `dock_needs_survey` the dock is done would cost us the observation permanently. Rejected rows are kept with a reason: a pattern of plausible junk from one address is the only evidence this surface is being abused, and it is invisible if each row is deleted as it is dismissed. **What remains is deployment** — `infra/aws/` has never been applied, so `portal.lmxit.com` does not resolve and the driver-app link still 404s in the hand of anybody who taps it |
...
| **AGT-4** | Decision explanation for the operator console — why this order was held | `BUILT` | `app/record/explain.py` — `explain_order`, `Explanation`, `Fact` — surfaced by `GET /operations/orders/{id}/explanation` in `app/api/routes.py`. Every explanation cites `REC-1`'s decision log rather than narrating: each line carries the snapshot it came from, and where the record is silent it says so rather than reasoning from what the order looks like now. An explanation assembled from current state would be a plausible story about the past, and a dispatcher cannot tell a plausible story from a true one — which is exactly when they stop trusting the console. **This row cited no artifact until 25 Sep** and was the only `BUILT` row in this table that named nothing a reader could open; the staleness check found it on its first run |
