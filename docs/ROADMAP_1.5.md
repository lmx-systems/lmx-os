...
| **PRD-1** | Batch value by node class | `NEW` | Reproduces the sensitivity finding: +3–7% at high-frequency shops, **+195–271% at warehouse and transfer nodes** |
| **PRD-2** | Trip cost versus order value | `NEW` | Flags the $5.88-part-on-a-35-mile-trip class. A join and a rule, not a model |
| **PRD-3** | Shrinkage baseline library | `BUILT` | `lmx-dwell/dwell/baseline.py` — promote it. Beats a global model on held-out data |
| **PRD-4** | Evaluation harness — time split plus held-out receivers | `BUILT` | `lmx-dwell/dwell/train.py` — promote it. Losing on unseen receivers blocks release |
...
| **PRD-6** | Conformal intervals on the promise | `NEW` | Stated coverage holds on held-out data |
