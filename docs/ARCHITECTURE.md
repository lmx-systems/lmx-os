...
> period rather than from Hub 1's own operation. Affected passages are
...
- **Google Route Optimization API** (`app/optimizer/google_routes_client.py`):
  ...
  **Not yet verified against a live Google Cloud project** — no
  ...
  real `optimizeTours` call. Before **cutover** (not before Hub 1 — see
  the note at the top of this file): provision a service account,
...
- **ADP/Gusto**: not wired in at all - no client code exists for either,
  ...
  doesn't run payroll.
- **Elite EXTRA — not stubbed, entirely absent.** Listed here separately
  because it is a different kind of gap from everything else in this
  section: the others are built-but-unconfigured, where a stub swaps in
  until credentials land. This one has no interface, no adapter, no stub,
  and no mention anywhere in the codebase — yet as of July 2026 it is the
  ...
  here and a cutover date. Tracked as Phase 3.5 in `docs/ROADMAP.md`.
...
2. **SLA hold-window minutes** (`app/sla/engine.py`,
   ...
   still the first thing to recalibrate further once real data exists.
   Per-shop/per-hub overrides already exist via `active_rules` so retuning
   doesn't require a code change, just data.
   **Where that data actually comes from (July 2026):** not from Hub 1's
   own operation. Hub 1 runs on the Elite EXTRA scaffold, which dispatches
   as fast as possible and does no batching at all — deliberately, since
   paying Elite EXTRA to build hold logic would hand LMX's core
   differentiator to a competitor. So the scaffold exercises these hold
   windows exactly zero times. They get calibrated during the **shadow
   period**, where LMX OS plans against the same real orders in parallel;
...
2. Confirm the real Epicor payload shape with the first **signed
   ...
   warehouse/staging module is now a pre-signature qualification question,
   and the interim path into the scaffold is a 15-minute file drop rather
   than a direct connection — which eats a third of a 45-minute T1 promise
   before anyone even sees the order. That latency tradeoff is an open
   decision, tracked as D6.)
