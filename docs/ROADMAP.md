...
| L24 | Client referral programme (proposed August 2026 — not yet approved) | **A referral reward is not a credit, and the difference is load-bearing.** `InvoiceCredit` cannot carry one: `order_id`, `sla_tier`, `promised_by`, `delivered_at` and `minutes_late` are all NOT NULL and a referral has none of them, so reusing that table means making its evidence columns nullable and hollowing out the model whose own docstring says a credit a client cannot check is one they will ring up about. **The worse damage is to a number we are still waiting on:** `app/reporting/credit_exposure.py` reads `InvoiceCredit` as what missed commitments cost, explicitly to give `E11` a basis for the 100/50/25/0 credit percentages that "have no derivation at all" — so referral rewards landing in that table would inflate breach cost with marketing spend, and someone would tune a service-level remedy against it. (`invoice_detail_view` also sorts credits by `minutes_late.desc()`, which a referral has no value for.) Own table, own column beside `gross_cents`/`credit_cents`, own line on the statement. **Attribution is a code at signup, never a form that submits someone else's details.** An optional `referral_code` on `ClientSignupBody`. The "refer a colleague and we'll call them" shape is the obvious one and it is the one to refuse: it makes LMX hold contact data for a business that never approached us, which `R3`'s privacy policy — structured by whose data it is, businesses / recipients / drivers — has no category for, and it puts us cold-calling on a client's behalf. The cost of the cheaper design is real and worth stating: attribution only works if the referred party actually types the code. **The reward goes to the business, never to the person who clicked.** It lands on the company invoice via `client_id`. Not a modelling detail — the individual referring is an employee of the client, and paying a purchasing decision-maker to steer their employer's business to us is the shape of a kickback rather than a referral. `C4` already separates `client_users` from `Client` so the right behaviour is the natural one, but the terms and the portal copy both have to say it out loud. **Earned on the referred client's first delivered order, not on signup.** A signup is free to manufacture and a delivery is not; it is the same event `L17`'s headline metric already keys on (`approved_at` → first delivery, migration 0036), so nothing new has to be recorded to detect it; and a referral that never becomes revenue then costs nothing. **Fraud control is the approval gate, deliberately, not automation.** Every signup already needs a human decision (`C5`) and that is where rates are set — so ops sees "referred by X" on the pending-signup card and refuses a self-referral by eye. No dedupe heuristics, no identity matching: that is a 200-client problem being solved at one, the same call the offsite made on deferring gig intake automation. Recorded here so it is not later rediscovered as a gap. **The reward amount is a placeholder, not a number this row invents** — same convention as `PLACEHOLDER_SLA_TERMS` and `PLACEHOLDER_HOURLY_RATE_CENTS`. What a referral is worth is a commercial judgement of exactly `E11`'s kind and wants the same sign-off. **Blocked on `L8`, for a substantive reason rather than a procedural one.** Clause 5 is prices, clause 6 is credits-as-remedy-for-a-miss; a referral reward is neither, so it needs its own clause covering how it is earned, whether it expires, and that it accrues to the business. A reward appearing on an invoice with nothing in the terms describing it is the same defect `L8` already fixed for assent. **Naming (`CLAUDE.md`):** customer-facing framing is "refer another business to LMX" — never "the LMX Link referral programme", and never affiliate / partner / commission language, which reframes an operator as a vendor with a channel. Carriers refer; software companies run affiliate schemes. **The premise is weaker than it looks, and the scope follows from that.** Auto-parts distributors in one metro compete, so "refer another distributor to your delivery carrier" is asking a client to speed up a rival. The motions that actually work are narrower: **branch-to-branch inside a group** (`W3` notes a distributor can have forty branches) and **out-of-territory distributors**. A third and possibly stronger motion — a repair shop referring its *other* suppliers, since a shop has a genuine interest in every supplier being fast — is deliberately **not** in this row: shops are `Shop` rows with no portal login, so it needs a surface that does not exist. **Boundary note:** a new `app/referrals/` package must be classified in `tests/test_architecture_boundaries.py` or that test fails by design. Nothing here touches the dispatch engine. |


### LMX Link — money movement (T2.5 and T6)

**Added September 2026, because this scope was invisible to every count in this file.**
The money-movement design lives in `docs/LMX_LINK_PLAN.md` Part 3 and its milestones —
**T2.5 (Rating & Statement)** and **T6 (Money Movement)** — had no rows here, so work
landing against them moved no figure and appeared in no audit. `L25` shipped to `main`
before this section existed.

**The plan's own feature ids are not reused here, deliberately.** §3.3 numbers its
features `A1`–`A9`, `B1`–`B6`, `C1`–`C7`, `D1`–`D4`, `E1`–`E6`, `F1`–`F3`, `G1`, `H1`–`H4`
and `J1` — and in *this* file `A1` is push notifications, `E1` is Route Optimization and
`G1` is notification-listener intake. Importing them would have made every cross-reference
in both documents ambiguous. Each row below names the plan features it covers instead.

**T2.5 sits before T4, the live pilot** — real drops need real prices, and the tiered
pricing decision means every drop needs one computed at intake.

**This moves the LMX Link track's headline figure from 90% to 59%, and that is not a
regression.** The intake work (`L1`–`L24`) is exactly as finished as it was; 13 items of
real, previously untracked scope have joined the denominator. Read the two as separate
tracks — **intake, 19 of 21** and **money movement, 1 of 13** — because averaging a
nearly-finished track with a barely-started one describes neither.

| # | Item | Why it matters |
|---|---|---|
| ~~L25~~ | ~~Rate card as versioned, effective-dated data~~ | **Done, September 2026** — plan features `A1` and `H1`. `client_rates` is append-only: `effective_from` carries when a version starts applying, the unique constraint moved from `(client, tier)` to `(client, tier, effective_from)`, and `orders.rate_version_id` records which version produced each fee. **The defect was narrow and easy to miss:** orders were never at risk, since `fee_cents` and `fee_breakdown` are frozen at ingestion — what a rate edit destroyed was the *card's own* history, so "what was this client's T2 rate on 15 August" had no answer. Pricing takes the newest version whose date has passed, so a future date **schedules** a change rather than performing one. Migration `0045`, 5 tests. `H1`'s immutable pricing audit trail falls out of this rather than needing its own work. |
| L26 | Distance, zone and quote at intake | Plan features `A2`, `A3`, `A5`. `billable_distance_mi` computed once at intake and stored; zone assigned from distance; `quoted_amount`, `rate_card_id`, `rate_version_id` and `zone` populated on the order. **Half-present already:** `Order.quoted_amount_cents` exists and nothing populates it at intake, and `client_rates.rate_per_mile_cents` has priced by distance since `F5` with nothing storing the miles it charged for. Distance today is straight-line at a placeholder speed — `E1` is what turns it into road miles. |
| L27 | Volume discount tiers | Plan feature `A4` — standing route, committed programme, dedicated route. **The schema is buildable now; the numbers are not.** The tier values are a commercial decision owned by Rich and Matan, of exactly `E11`'s kind, and the repo's convention is to build the engine with openly labelled placeholders rather than wait or invent. |
| L28 | Billable events and adjustments | Plan features `B1`, `B3`, `B4`, `H2`. `DELIVERED` generates a billable event; wait time, failed delivery and cancelled-after-dispatch become adjustments; manual adjustments carry `applied_by`, timestamp and reason code, with an approver role required (`H2`). **`B2` is already done** — automatic SLA-breach credits shipped as `W3`, read from `client_sla_terms`; it is listed under T2.5 in the plan but needs no further work. |
| L29 | Statement and export | Plan features `C1`, `C2`, `C3`, `C4`, `C5`, `H3`. Billing cut-off and timezone rules, per-store breakdown, export to the fixed schema, and **reproducibility** — re-running a closed period must return identical figures. Reproducibility is the one with teeth, and `L25` is a prerequisite for it: a statement cannot be reproducible if the rate that priced it can be overwritten. Distinct from `C3` in this file, which is invoice *generation* and already ships. |
| L30 | Cost capture | Plan features `D1`, `D2`. `cost_actuals` on every order — driver cost, mileage, fuel, tolls — each tagged with `hub_id` and `vehicle_id`. `cost_actuals` has been in the order contract since §1.2 and is populated by nothing, so margin per drop is currently uncomputable. This is the input `E9`'s DPH validation and the unit-economics work both need. |
| L31 | Taxability flag on the statement line | Plan feature `G1` — **flag only, no calculation.** Determination, calculation, filing and nexus are bought, per §3.2's rule that LMX OS computes and bought systems execute. |
| L32 | Per-client rate overrides, surcharges and minimums | Plan features `A6`, `A7`, `A8`. **Two of the three are already built and mis-scoped as T6:** `client_rates` is keyed per client, so `A6`'s override is how the table already works, and `A8`'s minimum charge shipped as `minimum_charge_cents` with `F5`. What is genuinely absent is `A7`'s surcharge library — after-hours, oversize, multi-piece, return leg. |
| L33 | Return-leg and redelivery billing | Plan features `B5`, `B6`. `RETURNED_TO_HUB` and core/warranty return legs become billable; redelivery charging rules. The reverse leg itself exists as `W1`; what is missing is that none of it is priced. |
| L34 | Weekly and custom billing cycles | Plan feature `C6`. Monthly is `L29`; this is everything else. `C7` (an API replacing the file export) is explicitly Phase 2 at ~20 clients and is not tracked here. |
| L35 | Multi-client cost allocation and the cost-per-drop feed | Plan features `D3`, `D4`. Splitting cost across a commingled run, and the per-hub, per-client, per-zone, per-modality feed. **Gated on commingling actually existing** — which is `B2`'s second client, not an engineering task. Depends on `L30`. |
| L36 | Worker payout computation | Plan features `E1`–`E5`. Worker record, pay calculation, gig-overflow payout, payout file export, reimbursements. **Substantially built already:** real hours and federal overtime ship as `E13`, gig per-delivery pay as `A11`, and the `PayrollProvider` interface exists against a stub. What remains is the worker record's employment-type/home-state fields, reimbursements, and confirming the export shape against a real Rippling tenant (`B4`). Execution — withholding, filings, 1099s — is bought. |
| L37 | Reconciliation against the accounting system | Plan feature `J1`, added when the September 2026 decision reversed invoicing from bought to built. Once LMX OS issues the invoice, **two systems hold a number that must agree and nothing checks that they do.** QuickBooks Online is under consideration for this and only this; the vendor is unchosen and the work is unscheduled. |
...
**`L21`, `L22` and `L23` are core optimizer work, not LMX Link work.** They carry
the intake track's prefix because they were numbered in sequence after `L20`, but
per-stop ETAs, visit-level route sequencing and solver time windows all belong to
this section — which is why they are listed here rather than in the LMX Link table
above. **Count them as LMX OS**; including them in the Link track's completion
figure overstates it. They are deliberately **not** renumbered: `L21`–`L23` are
cited in eight places outside this file — `app/delivery/eta.py`,
`app/api/driver_routes.py`, `scripts/verify_route_optimization.py`,
`tests/test_optimizer_google_client.py` and `docs/NEXT_STEPS.md` — so renaming them
here alone would break every one of those, and an id is a durable handle before it
is a category label. Renumbering to the `E` series is a real option, but it is a
change to those five files as much as to this one.

| # | Item | Why it matters |
|---|---|---|
| ~~L21~~ | ~~Per-stop ETAs~~ | **Done, August 2026** — `app/delivery/eta.py`, migration `0040`. `Stop.eta` was declared, served to the driver app in every route payload, read in two places in `driver_routes.py` and **written nowhere** — so every stop's ETA was null forever, and `app/models/stop.py`'s claim that `arrived_at` vs `eta` is I1's ETA-accuracy ground truth had one side of the comparison structurally absent. **The optimizer's own visit times could not be used**, which is the interesting part: `optimizeTours` is called with `considerRoadTraffic` and returns a `startTime` per visit, but `accept_offer` re-sequences afterwards (HOT_SHOT to the front of each block, every pickup before every dropoff — which the solver's interleaved plan does not do), so copying planned timestamps onto the accepted route would produce ETAs that are wrong and non-monotonic, with stop 2 arriving before stop 1 in the driver's own list. Instead the walk follows the **accepted** sequence using the same `minutes_for_miles` / `PLACEHOLDER_STOP_SERVICE_MINUTES` placeholders the accept-gate, the portal estimate and the tracking page already use — the point being not that the model is good but that a driver, a recipient and a counter person can never be shown numbers derived three different ways. Refreshed on accept, arrive, complete and flag, anchored on the most recent of {latest location ping, last stop reached}, and never predicting into the past. **`Stop.eta` freezes at arrival, not completion** — a test written against completion caught it being rewritten 35ms after the fact, replacing a forecast with a description of the present. `planned_eta` (new column, written once) is what preserves the prediction, so a live-moving `eta` does not destroy the measurement it was supposed to provide. A stop with no coordinates gets no ETA **and neither does anything after it**, because arrival at stop 5 is unknowable without knowing where stop 3 is. Side effect worth naming: the recipient tracking page used to fall through to `promised_at` for anyone who was not the driver's current stop, so a route running an hour late told three of its four recipients that nothing had changed — they now get their own stop's ETA. **Still open:** carrying visit-level sequencing through `RouteOffer` so the solver's traffic-aware times can be used at all (its own piece of work, and the prerequisite for F6). |
| ~~L22~~ | ~~Visit-level route sequencing~~ | **Done, August 2026** — `RouteVisit`, migration `0042` (`route_offers.visit_payload`). The solver has always planned both legs of a shipment together, which is what makes its travel times and feasibility right; `_visit_sequence` then **deduplicated the two visits down to one order id and threw the drop ordering away**, because `RouteAssignment` modelled a route as a flat list of orders. `accept_offer` rebuilt it as "every pickup, then every dropoff" — legal, longer than the plan we costed, and the reason the solver's arrival times were unusable. Routes can now interleave: collect A, collect B, drop B, drop A. **Commingling survives it** — *consecutive* pickup legs at one shop collapse into one stop, non-consecutive visits to the same shop stay separate because the solver had a reason. **The finding that shaped it:** the solver receives `SLA_TIER_SKIP_PENALTY` (a million units for skipping a HOT_SHOT) but **`hold_deadline` is never sent as a `timeWindows` constraint**, so it has every reason not to drop a hot shot and no reason to schedule one early — the blunt sort in `accept_offer` was the *only* thing prioritising the premium tier. Letting the plan govern unqualified would have silently degraded a paid tier, so HOT_SHOT legs are still hoisted, and that override is now named rather than incidental. Hoisting **both** legs is strictly better than what it replaced: the old ordering made a hot shot's *delivery* wait behind every other collection, which is the opposite of the direct point-to-point trip being sold. Two fallbacks, both tested: an offer with no plan builds the old way (offers live for `job_offer_ttl_seconds`, so a deploy lands with real offers in front of real drivers), and a plan missing a leg falls back rather than building a stop the driver can neither finish nor escape. `_build_stops_unplanned` therefore stops running about two minutes after rollout. **Prerequisite work done for `F6`.** Still open: send `timeWindows` so urgency lives in the model and the hoist can go (ties to `E10`), then move `Stop.eta` onto the planned *intervals* — real road-network travel times instead of `minutes_for_miles` at an assumed speed. |
| ~~L23~~ | ~~Collection deadlines as solver time windows~~ | **Done, August 2026** — `SLA_TIER_LATENESS_COST_PER_HOUR`, `StopCandidate.collect_by`. The solver knew a HOT_SHOT must not be *dropped* (a million-unit `penaltyCost`) and had no idea when one was *due*, so it had no reason to schedule one early — which is why the blunt hoist in `accept_offer` was the only thing prioritising the premium tier, and why the solver's own arrival times were unusable as ETAs. `hold_deadline` now goes out as a `timeWindows` entry on the collection leg. **Soft, never hard**, for exactly the reason shipments are skippable rather than mandatory: an `endTime` the solver cannot meet makes the shipment infeasible, so it gets skipped, turning a late collection into *no* collection. `costPerHourAfterSoftEndTime` is per tier and deliberately far below the skip penalties — being an hour late must never cost more than abandoning the order, or the solver would rather drop a T3 than deliver it late. **Clamped into the request's global window, which is the normal case not an edge case:** the batch-hold queue releases an order *when its deadline passes*, so most orders arrive already overdue and their raw deadline sits before `globalStartTime`, which the API rejects — clamping says "as early as possible" and lets the hourly cost decide which overdue order goes first. The deadline used is `Order.hold_deadline`, the same value the portal reports as `collect_by`, so the solver optimises against the promise a counter person actually read off the confirmation screen. **The HOT_SHOT hoist deliberately stays** until this is verified live: nothing has ever made a real `optimizeTours` call (`E1`) and the stub does not model time, so "windows are honoured" is an expectation, not an observation. `scripts/verify_route_optimization.py` gained check 5 — two orders beside the same driver, separable only by deadline — which is the exact evidence that retires the hoist, and an offline test proves that check is satisfiable before anyone spends a paid API call on it. Lateness costs are placeholders in the same sense as the skip penalties and want tuning with `E10`. |
| ~~W11~~ | ~~One promise, shown and measured~~ | **Done, August 2026** — `app/sla/commitment.py`. **We showed a commitment we never measured and credited one we never showed.** `app/billing/credits.py` assessed a breach against a target computed from `ClientSlaTerm.delivery_target_minutes`, and that field reached no client-facing surface anywhere — so a client owed an automatic credit could not see the target it was measured from. Meanwhile `GET /client/orders` reported `hold_deadline` as `collect_by`, and the confirmation screen presents it as "we'll collect by 2:40 PM", despite `credits.py`'s own docstring calling that field *"ours and internal … when we must set off, not when the customer gets their part"*. Found while propagating `collect_by` into W5's search results. The delivery commitment is now defined once and read by both billing and the client views, with a test that compares the two figures directly rather than trusting they agree. It reports its **source** — `explicit` (something said out loud, which outranks a default), `tier_term`, or `none` — because "nobody wrote down what we owe this client" is not a time, and inventing one would either credit us against a number nobody agreed or promise a customer something we never said. `collected_at` (the pickup stop's `completed_at`) now sits beside `collect_by`, which makes the second promise checkable for the first time and will show it running late by construction. **Left open deliberately, and it is a decision not a build:** `hold_deadline` is the batch-release moment, so real collection lands after it by however long the drive takes — the shown collection commitment is optimistic by construction. What the collection promise *should* be, per tier, belongs beside `delivery_target_minutes` in the rate-card conversation (`E11`). |
| E1 | Verify the Google Route Optimization client against a live Google Cloud project | **Still open, but no longer the same task — one command away.** The real client (`app/optimizer/google_routes_client.py`) has still never made a live `optimizeTours` call; what changed is that reading it against the documented API first turned up four defects, so that call now verifies a mapping worth verifying rather than discovering these one at a time. **(1) The request had no objective function.** Vehicle costs default to zero in the API and none were set, so with skip penalties as the only costs every feasible plan scored identically and the returned sequence was arbitrary — we were paying for a solver and asking it to optimize nothing, while `considerRoadTraffic` bought accurate traffic for a route nobody minimised. Fixed with `costPerHour`/`costPerKilometer`, chosen to stay an order of magnitude below the smallest skip penalty so driving never becomes more expensive than abandoning an order. **(2) The request modelled only half the journey.** `StopCandidate.lat/lng` is the *shop* — the pickup — and it was sent as the shipment's only `deliveries` entry, so the solver was told every job ended on collection. Shipments now carry a `pickups` leg at the shop and a `deliveries` leg at the customer, which also means Route Optimization's atomic-shipment guarantee prevents a driver being given a collection whose drop doesn't fit. Required threading delivery coordinates through `HeldOrder` (and its Redis payload, backward-compatibly) and all four of its construction sites. **(3) A failed call was almost undiagnosable** — `raise_for_status()` discarded Google's error body, which for this API is usually the actual fix ("API has not been used in project X" vs. a missing `roles/cloudoptimization.user`), and tenacity without `reraise=True` replaced it with `RetryError`. Also added a retry predicate: it previously retried a 400, spending the 5s cycle budget twice to fail identically. **(4) The client was rebuilt every dispatch cycle**, running blocking `google.auth.default()` on the event loop, discarding the credential cache so every cycle did a token-endpoint round-trip, and leaking an `httpx.AsyncClient`. Now a process-wide singleton, same shape as the geocoder's. **Remaining work is one command:** `.venv/bin/python -m scripts.verify_route_optimization` makes a single real call against a two-cluster scenario whose right answer is knowable in advance, and checks label round-tripping, that nothing was skipped, that both legs were sent, and — the one that matters — that each driver got the cluster beside them, which is what catches an objective function being ignored while every other check still passes. A test asserts that scenario is satisfiable, so the single paid run isn't spent on a broken assumption. Needs the Route Optimization API enabled and `roles/cloudoptimization.user`. **`docs/E1_ROUTE_OPTIMIZATION_ACCESS.md` is the request to hand to whoever owns the Google Cloud org** — the two commands, why the Maps key already in use does not cover this (Route Optimization is IAM-gated, not an API-key product), and why not to answer it with a service-account key. |
...
| ~~E6~~ | ~~Confirm the Learning Loop's flag-type naming convention with a driver-app stakeholder~~ | **Done — signed off at the July 28 2026 cofounder alignment**, and this row was simply never struck through. The decision log at the top of this file records it as *"Signed off as-is (`hold_window_too_short` / `hold_window_too_long`). Broadening the annotation vocabulary stays as I3."* `HOLD_TOO_SHORT_FLAG`/`HOLD_TOO_LONG_FLAG` in `app/learning_loop/detection.py` are therefore a confirmed contract, not a proposal. **Worth knowing because it unblocks something:** `I3` reads "coordinates with E6's naming sign-off", so I3 has been buildable since July and nothing said so. Found in the August 2026 build audit, which counted this row as open. |
...
| ~~E13~~ | ~~Real hours-worked/overtime calculation + admin payroll-run endpoint~~ | **Renumbered from `E11`, August 2026** — the id collided with the still-open "Confirm the SLA delivery targets and credit percentages" row directly above, so a citation of `E11` resolved to one done item and one open one depending on which row the reader landed on. `E11` stays with the SLA row because that is what every reference outside this file already means: `app/reporting/credit_exposure.py`, `app/api/routes.py`, `app/schemas/client_auth.py`, `dashboard/src/components/MeasurementPanel.tsx`, `scripts/set_client_sla_terms.py` and two integration tests all cite `E11` for the credit percentages. Nothing cited it for payroll, which is why this is the half that moved. **Done** — `app/payroll/hours.py` replaces the old route-span earnings heuristic with real on-duty hours from a durable `driver_shift_events` log, plus federal 40hr/week overtime for `w2` drivers. New `POST /admin/payroll/{hub_id}/run`. Known gaps: no state-specific daily-OT rules, and a workweek split across two pay periods only sees hours visible in the period being computed — see `docs/NEXT_STEPS.md` item 22. |
...
| ~~A1~~ | ~~Push notifications~~ | **Partially done** — `app/messaging/push_client.py` (Expo push, same "unconfigured -> stub" shape as Twilio) + `app/messaging/job_offer_notifications.py`, called from `app/optimizer/service.py` the moment a `RouteOffer` is actually committed, notify every registered, non-revoked device for that driver. `POST /driver/me/push-token` registers a device's Expo push token. Driver-app side: `notifications/registerForPushNotifications.ts` requests permission and registers the token after sign-in. ~~Real gap: the driver app has no EAS project id configured yet (`app.json`)~~ **September 2026: that gap is closed.** `app.json` carries `extra.eas.projectId`, which is exactly where `registerForPushNotifications.ts` reads it, so registration no longer no-ops. Nothing stands between `EXPO_PUSH_ENABLED` and a real send. **Not the same as verified:** a token minted in Expo Go is not one from this project, so proving it end to end needs the dev build `A6` is about — which is also what background geofencing needs (`docs/BACKGROUND_LOCATION_CONSENT.md` §101: neither permission tier is verifiable in Expo Go). |
...
| ~~A6~~ | ~~Mobile app store deployment pipeline~~ | **Partially done** — `driver-app/eas.json` (real development/preview/production build profiles + submit config), `eas-cli` as a devDependency, `.github/workflows/eas-build.yml` (manual dispatch or a `driver-app-vX` tag, gated on an `EXPO_TOKEN` repo secret). ~~Same "real config, unexercised against a live account" status as `infra/` (S3) — no Expo account/project exists yet, so `app.json`'s `extra.eas.projectId` doesn't either. One `eas init` closes this and, as a side effect, unlocks A1's push notifications too~~ **September 2026: the Expo project exists.** `app.json` carries `extra.eas.projectId` under owner `sourabhmiglani`, so `eas init` has been run and `A1`'s client-side blocker is closed with it. **What remains is Apple, not Expo:** an iOS dev or internal build needs an Apple Developer account for device provisioning, which is the same thing `0.8` is open on. An **Android** development build needs none of that and works today — EAS generates the keystore — which makes Android the only handset that can currently demonstrate background geofencing (`DRV-1`) or a real push token on a device (see `driver-app/README.md`'s EAS section) — no code change needed either way. |
...
| ~~A12~~ | ~~The driver's own sessions~~ | **Done, September 2026** — `DevicesScreen`, reached from Profile. `GET` and `DELETE /driver/me/devices` shipped with the first auth commit, were tested, and had **no caller**: an admin could see and revoke a driver's sessions and the driver could not, which is the asymmetry rather than a missing feature. `is_current` had existed all along and only a screen has any use for it. Revocation lands on the revoked device's *very next request* — `get_current_driver` checks the revoked set, not just the refresh path — so a phone left in a van can be ended before the van arrives. **The current phone is listed with no sign-out button**, and that is the load-bearing decision: revoking the session the offline outbox drains through does not empty the queue, it strands it — every flush then 401s and `refreshOnce` has nothing left to refresh with. `DRV-4` made a 401 transient precisely so a shift queued in a dead zone survives, and that reasoning holds only while the driver can still authenticate. The same discovery found Log out unguarded — a bare `onPress={signOut}` with no confirmation and no count; `logOutWarning` now reads the queue first and stays silent when it is empty, since a dialog on every log out is one drivers learn to dismiss. Found by `tests/test_no_unreachable_routes.py`. |
...
| # | Item | Why it matters |
|---|---|---|
| ~~C3~~ | ~~A real billing/invoicing system~~ | **Partially done** — `app/billing/service.py`'s `generate_invoice()` sweeps a client's delivered, priced, not-yet-billed orders in a date range into a new `Invoice` (`invoice_id` on `Order` prevents double-billing across periods); admin-triggered via `POST /admin/clients/{client_id}/invoices/generate`. Client portal has a real Invoices tab: a list + itemized detail view with a print-friendly layout ("Print / Save as PDF" via the browser, not a server-generated PDF binary — a deliberate scope cut, same class of decision as the payroll module's Rippling gate). **September 2026 — an operator can now actually raise one.** Until this, nothing in the system ever had: `generate_invoice` has exactly one call site, `POST /admin/clients/{id}/invoices/generate`, and that endpoint had no caller in any front end, no scheduler and no script — while the client portal shipped a complete viewer (list, detail, PDF) reading a table nothing could write. **A read-only feature is the mirror image of a write-only column** and just as empty. It was also passing the reachability check on a collision: `/generate` matched two dashboard comments about `docker/generate-env-config.sh`, a shell script, and only became visible once comments stopped counting as callers. `ClientInvoicesPanel` sits below the rates and terms panels — price, then promise, then the bill computed from both. **The list is the safety, not a nicety:** `NoBillableOrdersError` protects the *orders* from being billed twice, not the operator from believing a period is unbilled when it is not — two adjacent periods both succeed and produce two real invoice numbers, so the screen refuses to raise anything until the existing statements have loaded and warns on an overlap. `GET /admin/clients/{id}/invoices` is new. Real gap still open: payment collection (a real processor, e.g. Stripe Connect) is explicitly out of scope here, same as B4 for driver payroll — this only produces a statement of what's owed. |
...
| # | Item | Why it matters |
|---|---|---|
| ~~W1~~ | ~~Returns & core pickups as first-class work~~ | **Done** (PRs #13–#16, `migrations/0024`–`0025`). `app/models/return_item.py` is the reverse leg the system couldn't model before: a core moves `expected → collected → returned_to_shop`, with `not_ready`/`cancelled` branches. Slice 1 — piggyback capture at the delivery visit (created at ingestion from a `return_manifest`/`core_return` flag, collected on the dropoff). Slice 2 — a counter-person "cores ready" flag for standalone accumulated returns (`origin_order_id` nullable). Slice 3 — the return-to-shop leg (driver drops cores at the shop; ops manual mark). Slice 4 — the counter-facing "awaiting pickup, with age" list (`age_hours` on every view), the `not_ready → ready_for_pickup` reschedule, and the client-portal **Returns** tab (list + flag-cores form). Per-shop readiness *prediction* remains a later intelligence-layer follow-on (uses I1 data), as scoped. **September 2026: only the client third of the front end had shipped, and the other two thirds are built now.** `tests/test_no_unreachable_routes.py` found that `collect-return`, `return-not-ready` and `return-to-shop` have no driver screen and `mark-returned` and `reschedule` have no ops panel — so a counter person can flag cores ready, and the driver who arrives to collect them has no button while the operator who closes the loop has no list. The customer-facing half is the half that demos, which is exactly why this went unnoticed through a `Done`. Both are built: `ReturnsPanel` in the driver app (decision in `stopReturns.ts`, 9 tests) and `ReturnsPanel` in the dashboard. **Two backend changes were needed first**, neither visible from outside: `StopView` carried no returns field, so the app could not have known when to show a button; and all three driver endpoints had to become idempotent before they could sit behind `DRV-4`'s retrying outbox — `collect-return` with a manifest **created a second core on every retry**, and a phantom core is a part a shop is owed that was never in the van. |
...
| ~~W3~~ | ~~SLA-breach invoice credits~~  **Made visible, August 2026** (`app/reporting/credit_exposure.py`, `GET /operations/credit-exposure`): W3 made a breach cost something but nothing showed the total, so a credit only appeared on one invoice, for one client, after billing ran — a month of breaches read as **zero** until somebody generated a statement. The report has two halves and the second is the point: *issued* is history from `InvoiceCredit`, *accruing* is delivered work not yet invoiced that would breach if it were, computed by calling the **same `assess_credits` invoicing calls on the same candidate set** — a test asserts the forecast equals the bill. Issued credits are windowed by **delivery date, not invoice date**, or every credit would move into whichever month billing happened to run. Each tier carries its configured credit percentage beside the money, which is what makes this an `E11` input rather than a figure; where clients disagree on a tier's terms it reports no percentage rather than picking one. Orders with no commitment are counted as **unknown** cost rather than zero (`W11`), and unpriced ones separately, since an order that will never be billed is a bigger problem than the credit it did not produce. | **Done, August 2026** — `app/billing/credits.py`, `app/models/client_sla_term.py`, `invoice_credits`, netted into the statement. **The finding: "late" was not computable.** W3 asks for credits "computed from order-level SLA outcomes" and the outcome did not exist — `app/sla/engine.py` defines HOLD windows (when we must set off), `hold_deadline` is ours and internal, and `promised_at` is only populated when a source hands us one, which for an LMX-owned order is never. A credit schedule alone would have been a penalty with no trigger. `client_sla_terms` supplies the missing half **as contract data per client and per tier, not a constant chosen in a Python file** — LMX has no company-wide delivery SLA written down anywhere, and hardcoding one would have invented our service level. Measured from receipt, because that is the moment a client can point at. Three rules, each a way this could quietly be wrong: **a tier with no term is reported as unassessable rather than clean** ("we owe nothing" and "nobody wrote down what we promised" are different answers); an explicit `promised_at` beats the tier default (something said out loud outranks a default); and **a credit never exceeds the fee**, since crediting more than was billed turns a statement into a payment. A row per breached order with the evidence frozen — what was promised, what happened, how late — because "which ones?" is the first question and an aggregate answers it with "check your own records". Invoices now carry gross, credits and net separately: a statement showing only a net is one a client cannot reconcile, and one that hid the credit would also hide that we missed something. **September 2026 — an operator can now read the terms back.** `GET` and `PUT /admin/clients/{id}/sla-terms` had existed since W3 shipped and had no caller: terms were set by `scripts/set_client_sla_terms.py`, and after that *"what did we agree to?"* lived only in the database — the question a client asks when they dispute a credit. `ClientSlaTermsPanel` sits directly beneath `ClientRatesPanel` because price and promise are one contract and a credit is a percentage of the fee the rate card produced. **A tier with no term reads "no promise", never a dash**, so an absence cannot be mistaken for missing data and invite somebody to invent a term; `PLACEHOLDER_SLA_TERMS` appears only as greyed input placeholders and is never pre-filled, since a placeholder somebody saves becomes a term and `E11`'s whole point is that it is not one. The client picker carries `sla_term_tiers` beside `rate_tiers` — two zeroes that do not mean the same thing: no rate is always a fault, no term is uncredited by design. |
| ~~W4~~ | ~~Driver-visible scorecard~~ | **Done, August 2026** — `build_driver_scorecard`, `GET /driver/me/scorecard`, `ScorecardScreen`. Two metrics chosen deliberately rather than everything available: **their own DPH and their own ETA error, each beside the same figure for their hub.** **The "identical computation" requirement is enforced structurally, not asserted:** `_deliveries_per_hour` and `_eta_accuracy` are the *same* functions the fleet-wide `I4` report calls, narrowed by a `driver_id` — one implementation and a filter, with a test asserting the driver's own figure equals what the fleet computation produces for them. Three decisions the trust framing forced. **The fleet median includes this driver** — "everyone except you" is a subtly adversarial number and not what a shared standard means. **The comparison is withheld below three other drivers on shift**, because with one colleague a team median plus your own figure *is* that colleague's figure — a privacy leak between drivers, not a statistical nicety; own figures still show. And **scoped to the driver's own hub**, since density and geography differ and comparing across hubs is a comparison to a different job. A driver with no finished shift gets an honest refusal rather than `0.0`, which would tell a new starter they are performing at zero. **Ratings and flag rates are deliberately absent** and a test pins that down: both exist per driver, and a recipient rating shown back is the sharpest edge in this data — one bad rating lands hard and recipients rate for reasons outside a driver's control. The driver id comes from the token with no parameter to override it. No score, no grade, no ranking; the definitions are on the screen, because a metric a driver cannot check the meaning of is not a shared standard. Ratings remain a separate decision if you want them later. 8 tests. |
| ~~W5~~ | ~~Counter-person order status lookup~~ | **Done, August 2026** — `GET /client/orders` gains search, a status filter and paging; `OrdersTable` gains the search box. **The premise turned out to be wrong in a useful way:** the row said this reopens C4's "one login per client" decision, but C4 already ships a `member` role with read-only access to the company's orders, so a counter person can have their own login today and no decision was needed. What was actually missing was findability — the endpoint **returned every order the client had ever placed**, unbounded and unsearchable, which is both a full scan that grows forever and useless for finding one order among thousands. `q` matches our reference, **their** reference, the shop, the delivery contact and the address; their own reference matters most of the five, because that is the number on the paperwork in front of them. LIKE metacharacters are escaped — without that, searching `%` matches every order and `_` matches any single character, so the box silently stops filtering exactly when someone is trying to narrow a long list. The total counts the *filtered* set, and the UI says "showing 1–50 of 312", because a page that does not admit it is one reads as the whole list and is how somebody concludes an order is missing. **And it closes the ETA half of the story:** the summary view carries `collect_by` (the commitment the confirmation screen showed) and `estimated_delivery_by` — the route-aware `Stop.eta` once the order is on a driver's route, a straight-line estimate before then. Same number the driver and the recipient see, which is the whole point. Params are `Annotated` so a direct call gets real Python defaults; the older `= Query(default=...)` form hands test callers a `Query` object that fails downstream with an AttributeError rather than at the call. |
...
| R3 | Privacy policy & data-handling/retention policy | **Drafted and served, awaiting counsel and four retention decisions (August 2026).** `app/legal/content/privacy.md`, structured by whose data it is rather than by data type — businesses we deliver for, people receiving a delivery, and drivers — because a recipient never agreed to anything with us and their requests mostly route back through the sender. The inventory was written from the schema, not from memory: every field it names is a column that exists. **The retention section is the part with teeth,** because a stated period nothing enforces is the same defect as proof of delivery nobody checks. Two are enforced — driver location trails at `LOCATION_PING_RETENTION_DAYS`, pruned by `POST /internal/retention/prune` (`app/legal/retention.py`), and tracking links via `tracking_link_grace_hours`. **Two more were built, August 2026** — message/call metadata at `COMMUNICATION_RETENTION_DAYS` (metadata only; call content is never recorded), and declined applications at `DECLINED_APPLICATION_RETENTION_DAYS`, which deletes the application and the inactive login signup created. That one needed migration `0041` for `clients.rejected_at`: `updated_at` was the tempting shortcut and would have let any future write to the row silently restart somebody's retention clock, and a rejection recorded before the column existed is **never guessed at** — it is reported as `skipped_undated` so a stuck row is visible. It also refuses to delete a rejected applicant holding orders, invoices or shops: that combination is a bug upstream, and deleting is the wrong response to a surprise. **Two remain unenforced** — proof-of-delivery images and driver documents, both object storage, where the mechanism is a bucket lifecycle rule rather than an application loop. Those two are now **bracketed placeholders in the policy text**, so `test_a_document_cannot_be_published_with_a_hole_still_in_it` refuses publication until either the rule exists or the sentence is softened. Note the interaction with `R1`: **proof retention must outlast the claim window**, or we delete the photograph before the client can still claim on it. Still gates Phase 9; `docs/LEGAL_BRIEF.md` has the decision list. |
...
| ~~F7~~ | ~~Client- and ops-facing analytics dashboards~~ | **Done, August 2026** — `MeasurementPanel` (ops), `PerformancePanel` + `GET /client/performance` (client). **Three reporting endpoints had zero consumers**, two of them built in the preceding fortnight: `GET /operations/scorecard` (`I4`), `GET /lmx-link/scorecard` (§3.4), and no ops view of credits owed at all. The data layer had been built twice and nothing surfaced it — the same captured-and-never-read pattern that made `I4` worth doing, one layer up. Ops now sees both scorecards, with a refusal rendered **as a sentence rather than a blank or a zero**, since "no data yet" and "nothing records this" need different actions and 0% on an empty table is what gets quoted. Clients see their own delivery volume and on-time rate, which `F7` rightly calls the retention proof rather than an ops nicety: a distributor on per-drop pricing has given up their own view of a fleet they used to run. That figure is `_sla_hit_rates` narrowed by `client_id` — **the same function ops reads and, through `app/sla/commitment.py`, the same promise billing credits a breach against**, so a client can never be shown 98% while being credited for a miss; a test asserts the two agree. Their own figures only, with no cross-client benchmark, which would disclose one customer's performance to another. Readable by a `member`, because a counter person fielding "are they any good" needs the answer as much as the owner. **Built without the driver leaderboard this row lists**, deliberately: `I4` computes DPH per driver-day and never keys it to a named driver, and `W4` shows a driver their own figures against an anonymised hub median with a de-anonymisation guard. A ranking would undo both by accident, and belongs to a decision rather than a dashboard feature arriving by default. Caught on the way: `Order.sla_tier` is an `SLATier` member when loaded from Postgres and a plain `str` on an in-memory object — both hash identically because `SLATier` subclasses `str`, so the difference is invisible until `.value` raises on one or renders `SLATier.T2` on the other. `_tier_label` handles both. Still open: cost-per-drop trend. **CSV export landed the same week** — `app/reporting/csv_export.py`, `GET /client/orders/export.csv`: a client's whole order history, **streamed rather than paged**, because a page of a ledger is not a ledger and silently truncating a data export is the failure it exists to avoid. **The interesting part is formula injection.** Excel and Sheets execute a cell starting `=`, `+`, `-`, `@`, tab or CR, and since `F13` one column is free text written by an **unauthenticated stranger** holding a tracking link — so without a guard a recipient chooses what runs on the distributor's machine when they open their own export. Neutralised with a leading apostrophe, **text only**: prefixing a negative number would break the arithmetic the export exists to enable. The generator opens its **own** session via `session_scope()`, because a streaming response outlives the request-scoped one, and the client lookup happens *before* streaming starts, since once bytes are on the wire an exception cannot become a clean status. Verified over real ASGI streaming as well as at the function level. |
...
| ~~F13~~ | ~~Ratings & feedback capture~~ | **Done, August 2026** — `app/models/delivery_rating.py`, `app/tracking/ratings.py`, migration `0043`. A one-tap 1–5 score plus an optional comment, captured from the recipient's own tracking link and surfaced to the client on the order detail. **The row said "prompt to the shop", which does not survive contact with the data model:** `Shop` is the *pickup* location — the client's own branch, which experiences a collection and never sees the delivery. The party who experiences it is the recipient at the door, who is already holding a link that stays live for `tracking_link_grace_hours` afterwards. So `rated_by` is recorded on every row from day one and the recipient path is what is built; a client-side rating is later an insert with a different author rather than a migration and a reinterpretation of existing rows. **The window is the token's own life** — no second expiry to keep in step, and `resolve_tracking` enforces it for free. A second submission **edits the first** (unique on `order_id, rated_by`), so a count of rows stays a count of people, while `first_submitted_at` survives the edit. Only a *delivered* order is ratable: a failed delivery has real signal but it is a different question, and one score covering both means nothing — exceptions keep their own channel in `flag_stop_issue`. Three validation layers, deliberately: the schema returns a useful 422, the service protects non-endpoint callers, and a CHECK on the table is what stays true if a row is ever written by hand (verified firing). **Adding to `TrackingView` was a disclosure decision**, since that schema documents itself as the exhaustive list of what a stranger with a URL can learn — the two fields added describe only the reader's own action, and a test asserts no driver, client or route identity crept in with them. **Nothing aggregates by driver, on purpose:** the same numbers read as a per-driver ranking are the "camera pointed at me" `W4` warns against, and that view belongs to the conversation `W4` frames rather than falling out of a capture mechanism. Feeds `I3`'s annotation vocabulary. 19 tests. |
...
| ~~W10~~ | ~~Package identity & scan-at-pickup verification~~ | **Done** (PR #11, `migrations/versions/0023_parcels.py`) — a real `app/models/parcel.py` with a uniqueness constraint, and `ScanParcelBody{barcode: str}` verified against the order at the pickup stop (`app/api/driver_routes.py`); the old bare-count `ScanParcelsBody` survives only as the manual "can't scan, confirm by hand" fallback. `tests/integration/test_parcel_scanning.py` covers it. **The open label decision was handled well rather than forced:** the model is ownership-agnostic — `barcode` holds either an LMX-generated code or the distributor's own pick-ticket value (`app/ingestion/service.py` takes it from the payload when present, generates one otherwise), and the verification path is identical either way. So the printer-vs-scan-existing hardware call stays deferred and reversible, which is what W8 should decide. A2's scanner now has something to read, and `WRONG_PART` is catchable at pickup rather than at the customer's door. |
| W9 | Shadow-mode comparison engine & cutover scorecard | The real shape of shadow mode per session decision D3: every initial customer engagement runs live on the Elite EXTRA scaffold while LMX OS **decides in parallel on the same orders**, and the two are compared until a scorecard passes and that engagement cuts over. Needs: a parallel decision path that records what LMX OS *would* have done without acting; per-order divergence capture (the session is explicit that aggregate metrics look fine while the two systems agree — the divergent orders are the entire point); and a nine-metric scorecard — drops per driver-hour, T1 on-time rate, batch rate, hold-release integrity, miles per drop, re-plan speed (<5s at real volume), human touches, decision divergence with outcome delta, and data completeness. Also a **sales asset**: "we transition only when our OS beats the baseline on your own orders." ~~Open decisions for D3: the thresholds, the minimum consecutive passing weeks, and the weekly review owner.~~ **All three were answered at the July 28 2026 cofounder alignment and this sentence was never updated** — see D3 in the decision log above: the bar is *strictly beat* drops/driver-hour, *no worse* on T1 on-time, hold-release integrity and <5s re-plan, with negative-outcome divergence trending to zero; **two consecutive weeks** each clearing a minimum order volume; **founder-owned** weekly review initially. Only the absolute numbers remain open, and they *calibrate from shadow data*, so they cannot gate building this — the engine has to run before there is anything to calibrate against. **W9 is fully specified and buildable now.** **The control-group half now exists** — tracked under LMX 1.5 as `EXP-0` (`docs/ROADMAP_1.5.md` Phase 2), where it is explicitly the *weaker* comparator that `EXP-1`'s randomised control arm supersedes — `app/baseline/` + `scripts/analyze_baseline.py` reduce a driver-activity export and a customer stop-invoice export to the historical baseline the scorecard is scored against (drops per driver-hour, miles per drop, on-time rate, batch rate, cost per stop, and the two-file reconciliation). This is the *comparator*, not the comparison engine: the four metrics that measure a live system — re-plan speed, decision divergence, hold-release integrity, human touches — are unobtainable from a historical file and still come from `app/shadow/`. Built against no sample data, so the CSV schema is a runtime input: columns auto-detect, `--map` binds what auto-detection misses, and an unresolved required column is a hard error rather than a guess — a wrong baseline is worse than no baseline, since nothing downstream can detect it. Every figure carries its denominator and its coverage, and a metric whose column is absent reports n/a instead of zero. |
...
| ~~I4~~ | ~~Descriptive analytics on the captured truth~~  **Extended August 2026 with offer outcomes and decline reasons.** `RouteOffer.decline_reason` was written only when a caller supplied one and **the driver app never did** — so unlike `planned_eta`, which was merely unread, this column was dead end to end and a report on it would have shown nothing forever. Making it real needed both halves. **Declined and expired are reported separately**, because they mean opposite things and get confused: a decline is an answer, an expiry is silence — most likely the driver never saw the offer, which is the likelier failure while push notifications are stubbed. A high decline rate is a pricing or routing problem; a high expiry rate is a delivery problem. Reasons are rated over *declines*, not over all offers, or they would look rarer the more offers were accepted. **The prompt is deliberately after the decline, not part of it** (`POST /driver/offers/{id}/decline-reason`): a driver has `job_offer_ttl_seconds` to answer and the orders are held until they do, so making them pick from a list first would leave work unassigned while they hesitated — trading dispatch latency for a data point. The decline still fires on one tap and releases the work; an abandoned prompt yields a decline with no reason, exactly today's behaviour. A reason cannot be attached to an *expired* offer, since that would be claiming a decision nobody made. Five-value vocabulary in `app/schemas/driver_app.py`, including `pay_too_low` — a reason nobody is offered is a reason nobody reports. | **Done, August 2026** — `app/reporting/operations.py`, `GET /operations/scorecard`. **The data had been accumulating and nothing read it:** `Stop.planned_eta` was added specifically so ETA accuracy could be scored and was read in **zero** places, `arrived_at` in one. Four measurements, all from records that already existed. **DPH per driver-day** as a distribution rather than a fleet average, because `E9` asks whether 2.5 is real and one average answers an easier question — a long shift with few drops and a short one with many average to something plausible while describing neither. **SLA hit rate by tier**, measured through `app/sla/commitment.py`, the same function billing credits against, so a hit rate and a credit can never disagree about what was promised. **Hold-window flag rate**, denominated in completed deliveries rather than flags (a rate over flags is 100% by construction). **ETA error**, signed rather than absolute — consistently early means the model is pessimistic and clients are quoted worse than reality, which is not the same failure as late. Several deliberate refusals in the arithmetic, each the flattering direction if got wrong: an **unclosed shift is dropped** rather than closed at `now` (that would divide today's few drops by twenty minutes and make the assumption look validated); `en_route` **counts as on duty**; a shift under 15 minutes is a toggle not a denominator; a shift with **zero deliveries counts as zero** rather than being dropped, or the metric would measure how productive productive days are; and an order with **no commitment on file is excluded, not counted as a hit**, per `W11`. **Everything can refuse to answer** — on an empty database the honest output is four `not_measured` reasons that distinguish "no data yet" from "nothing records this", verified by running it. `Measurement`/`Rate` extracted to `app/reporting/measurement.py` now the Link scorecard is not the only consumer; that refactor is behaviour-preserving, its 17 tests unchanged. Deliberately **not** per named driver — that view is `W4`'s conversation, not a side effect of answering `E9`. **Unblocks `W4`.** 18 tests. |
