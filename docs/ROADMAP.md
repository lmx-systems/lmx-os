...
| # | Item | Why it matters |
|---|---|---|
| ~~S1~~ | ~~Real per-user authentication for the ops dashboard~~ | **Mostly done** — `app/ops_auth/`'s `OpsUserAuthMiddleware` replaces the old shared `X-API-Key` with a real per-account Bearer JWT (same password+JWT shape as the client portal); `dashboard/` has a real login screen. `scripts/create_ops_user.py` bootstraps accounts. Real gap still open: no role model - every ops user can do everything any other one can. |
