...
| L8 | Publish the terms | **BLOCKING the signup page going live.** `SignupPage.tsx` records `terms_version: 'v1'` and no v1 exists. `docs/TERMS_V1_DRAFT.md` is engineering's draft for counsel; three clauses can't be finished without R1 (liability), R3 (privacy) and W7 (training rights). Not linked from the form on purpose. |
...
| L10 | A sink that reaches a customer | Not built. The portal reads status from Postgres directly, which is enough while it is the only consumer. The outbound webhook (F4) plugs into `app/orders/sinks.py` rather than growing a second notification path. |
...
| L14 | Self-serve password reset | **Not built, and now the next gap.** A client who forgets their password has no way back in: only an admin at their own company can reset it, so a sole-admin lockout is unrecoverable without `scripts/create_client_user.py` run by hand. Depends on L13, which now exists. |
| L15 | Proxy-aware rate limiting | **Not built, and a production blocker.** `app/rate_limit.py` and `app/client_auth/signup_rate_limit.py` both key on `request.client.host`, the direct TCP peer — behind a load balancer that is the *balancer*, so the public signup limit becomes one shared budget the whole internet draws from. Documented as a known gap since S6; LMX Link made it live by adding the first public endpoint. Must read `X-Forwarded-For` with a trusted-proxy count before anything sits behind an ALB. |
| L11 | `en_route_drop` emission | The transition exists in the machine but nothing emits it — the driver app has no "departed the pickup" event, and stamping it at the pickup-completion instant would be a meaningless timestamp. F1's live location now makes a real signal plausible. |
| L12 | Keyed geocoding provider | Nominatim is a pilot choice. Volume or commercial terms force a swap; the interface and `GEOCODER_PROVIDER` make it a config change. Procurement, not engineering. |
