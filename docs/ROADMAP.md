...
| W4 | Driver-visible scorecard | **Unblocked as of I4 (August 2026).** The orchestrator now has per-driver-day figures to share, so "the identical metrics and definitions" finally has something on the other side of it. Note `I4` deliberately stopped short of a per-driver view, and `F13` did the same with ratings, so what remains is exactly the trust decision this row is about rather than the arithmetic. Previously blocked: The requirement is that the driver sees *the identical metrics and definitions the orchestrator sees*, and the orchestrator currently has **no per-driver metrics at all**: `app/reporting/` holds the LMX Link scorecard and the COD dispute report, neither of which is per driver. Building this now would mean inventing both sides at once, which makes "a shared standard, not a camera pointed at me" vacuous — there would be nothing to share with, and the definitions would be mine rather than agreed. Sequence is `I4` first. Note `F13` deliberately captures recipient ratings without aggregating them by driver, for this reason. Story DR-10: the driver sees *the identical metrics and definitions* the orchestrator sees. Explicitly framed as a trust decision, not a feature — "a shared standard, not a camera pointed at me." Folds into I4/F7's analytics work; the requirement is that the driver view is the same computation, not a separate reduced one. |
