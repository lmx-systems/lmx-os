...
| 6 | Decide event-bus vs. manual trigger for the Dispatch Optimizer *and* the Learning Loop's nightly job | Design doc calls for re-optimizing on "every meaningful event" (order released, driver status change, stop completed), and the nightly job needs an actual schedule. Both are currently manually-triggered endpoints only. | Not started |
